...
def test_help():
    ...
    assert "Convert and aggregate h5 vp files to daily/monthly vpts files on s3 bucket" in result.output
...
def test_e2e_cli(s3_inventory, path_inventory, tmp_path):
    ...
    with patch('pandas.Timestamp.now',
        ...
        with open(tmp_path / "nosta_vpts_20230311.csv", 'wb') as f:
            s3_inventory.download_fileobj('aloft', 'baltrad/daily/nosta/2023/nosta_vpts_20230311.csv', f)
        filecmp.cmp(path_inventory / "nosta_vpts_20230311.csv", tmp_path / "nosta_vpts_20230311.csv")

        # Compare resulting monthly file
        with open(tmp_path / "nosta_vpts_202303.csv.gz", 'wb') as f:
            s3_inventory.download_fileobj('aloft', 'baltrad/monthly/nosta/2023/nosta_vpts_202303.csv.gz', f)
        filecmp.cmp(path_inventory / "nosta_vpts_202303.csv.gz", tmp_path / "nosta_vpts_202303.csv.gz")
